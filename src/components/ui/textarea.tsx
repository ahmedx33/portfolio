...
export interface TextareaProps
    extends React.TextareaHTMLAttributes<HTMLTextAreaElement> { validations?: { success: boolean, message: string }[] }

const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(
    ({ className, validations, ...props }, ref) => {
        ...
        return (
            <>
                <textarea
                    ...
                    {...props}
                />
                <ul className={`${show ? "mb-2 mt-1" : "my-0"} transition-all duration-300 overflow-hidden flex flex-col gap-2.5 w-full pl-2`} style={{ height: show ? (validations?.length ?? 0) * 30 : 0 }}>
